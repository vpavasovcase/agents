...
This script uses the exact same code structure as main_vojo.py.
"""
...
async def test_gmail_draft_exact():
    """Test the Gmail MCP server draft email functionality using the exact code from main_vojo.py."""
    
    print("=== Gmail Draft Email Exact Test ===")
    ...
    # Create a simple agent with all MCP servers
    test_agent = Agent(
        model=llm_model,
    ...
    async with test_agent.run_mcp_servers():
        ...
        # Create a draft email with hardcoded values
        ...
        # Use the draft_email tool to create the draft in Gmail - EXACT CODE FROM main_vojo.py
...
if __name__ == "__main__":
    asyncio.run(test_gmail_draft_exact())
