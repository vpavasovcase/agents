        ...
        for message in result.new_messages():
            for call in message.parts:
                if isinstance(call, ToolCallPart):
                    # Handle args whether they come as a dict or as an object with args_json/args_dict
                    if isinstance(call.args, dict):
                        call_args = json.dumps(call.args)
                    else:
                        call_args = getattr(call.args, 'args_json', None) or json.dumps(getattr(call.args, 'args_dict', {}))
